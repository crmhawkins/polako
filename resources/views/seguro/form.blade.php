<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group mb-4">
        <label class="mb-2 text-left uppercase" style="font-weight: bold" for="proveedor_id">Proveedor</label>
        <select class="form-select choices" id="proveedor_id" name="proveedor_id">
            <option value="">Seleccione salon</option>
            @foreach ($proveedores as $proveedor)
                <option value="{{ $proveedor->id }}" {{ old('proveedor_id',isset($documento) ? $documento->proveedor_id : '') == $proveedor->id ? 'selected' : '' }}>{{ $proveedor->name}}</option>
            @endforeach
        </select>
        @error('salon_id')
            <span class="invalid-feedback" role="alert">
