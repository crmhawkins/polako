<form action="{{ $action }}" method="POST" enctype="multipart/form-data">
            ...
            <select class="choices w-100 form-select @error('admin_user_id') is-invalid @enderror" name="admin_user_id">
                @if ($usuarios->count() > 0)
                    ...
                    @foreach ($usuarios as $usuario)
                        <option data-id="{{ $usuario->id }}" value="{{ $usuario->id }}" {{  (isset($nomina) && $nomina->admin_user_id == $usuario->id) ? 'selected' : '' }}>{{ $usuario->name.' '.$usuario->surname }}</option>
                    @endforeach
                @else
    ...
    <div class="form-group mb-4">
        ...
        <input type="date" class="form-control @error('fecha') is-invalid @enderror" id="fecha" value="{{ old('fecha', isset($nomina) ? \Carbon\Carbon::parse($nomina->fecha)->format('Y-m-d') : '') }}" name="fecha">
        @error('fecha')
    ...
    <div class="mb-4">
        ...
        @enderror
        @if(isset($nomina) && $nomina->archivo)
            <div class="mt-2">
                <a href="{{ asset('storage/' . $nomina->archivo) }}" target="_blank">Ver archivo actual</a>
            </div>
        @endif
    </div>
