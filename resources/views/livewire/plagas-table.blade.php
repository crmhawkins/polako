...
<div>
        ...
        <div class="table-responsive">
             <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr class="clickable-row" data-href="{{route('plagas.edit', $plaga->id)}}">
                           <td>{{$plaga->alta ? \Carbon\Carbon::parse($plaga->alta)->format('d/m/Y') : 'N/A'}}</td>
                            <td>{{$plaga->caducidad ? \Carbon\Carbon::parse($plaga->caducidad)->format('d/m/Y') : 'N/A'}}</td>
                            <td>{{$plaga->descripcion}}</td>
                            <td>{{optional($plaga->adminUser)->name}}</td>
                            <td>{{optional($plaga->salon)->nombre}}</td>
                            <td class="flex flex-row justify-evenly align-middle" style="min-width: 120px">
                                <a class="" href="{{route('plagas.edit', $plaga->id)}}"><img src="{{asset('assets/icons/edit.svg')}}" alt="Editar plaga"></a>
                                <a class="delete" data-id="{{$plaga->id}}" href=""><img src="{{asset('assets/icons/trash.svg')}}" alt="Eliminar plaga"></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="text-center py-4">
            <h3 class="text-center fs-3">No se encontraron registros de <strong>Control de plagas</strong></h3>
        </div>
    @endif
</div>
@section('scripts')
    ...
    <script>
        ...
        function botonAceptar(id){
            ...
            Swal.fire({
                title: "¿Estas seguro que quieres eliminar este plaga?",
