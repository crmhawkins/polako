...
<div>
        ...
        <div class="table-responsive">
             <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr class="clickable-row" data-href="{{route('documentos.edit', $documento->id)}}">
                            <td>{{$documento->nombre}}</td>
                            <td>{{ \Carbon\Carbon::parse($documento->fecha)->format('d/m/Y') }}</td>
                            <td>{{optional($documento->salon)->nombre}}</td>
                            <td>{{optional($documento->usuario)->name}}</td>
                            <td class="flex flex-row justify-evenly align-middle" style="min-width: 120px">
                                <a class="" href="{{route('documentos.edit', $documento->id)}}"><img src="{{asset('assets/icons/edit.svg')}}" alt="Editar documento"></a>
                                <a class="delete" data-id="{{$documento->id}}" href=""><img src="{{asset('assets/icons/trash.svg')}}" alt="Eliminar documento"></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="text-center py-4">
            <h3 class="text-center fs-3">No se encontraron registros de <strong>Documentos</strong></h3>
        </div>
    @endif
</div>
@section('scripts')
    ...
    <script>
        ...
        function getDelete(id) {
            // Ruta de la peticion
            const url = '{{route("documentos.delete")}}'
            // Peticion
            return $.ajax({
                type: "POST",
                url: url,
