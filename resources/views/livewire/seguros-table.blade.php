...
<div>
        ...
        <div class="table-responsive">
             <table class="table table-hover">
                <thead class="header-table">
                    ...
                    <th class="px-3">
                        <a href="#" wire:click.prevent="sortBy('proveedor_id')">
                            PROVEEDOR
                ...
                <tbody>
                    ...
                    <tr class="clickable-row" data-href="{{route('seguros.edit', $seguro->id)}}">
                            <td>{{$seguro->alta ? \Carbon\Carbon::parse($seguro->alta)->format('d/m/Y') : 'N/A'}}</td>
                            <td>{{$seguro->caducidad ? \Carbon\Carbon::parse($seguro->caducidad)->format('d/m/Y') : 'N/A'}}</td>
                            <td>{{optional($seguro->proveedor)->name}}</td>
                            <td>{{$seguro->descripcion}}</td>
                            <td>{{optional($seguro->adminUser)->name}}</td>
                            <td>{{optional($seguro->salon)->nombre}}</td>
                            <td class="flex flex-row justify-evenly align-middle" style="min-width: 120px">
                                <a class="" href="{{route('seguros.edit', $seguro->id)}}"><img src="{{asset('assets/icons/edit.svg')}}" alt="Editar seguro"></a>
                                <a class="delete" data-id="{{$seguro->id}}" href=""><img src="{{asset('assets/icons/trash.svg')}}" alt="Eliminar seguro"></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="text-center py-4">
            <h3 class="text-center fs-3">No se encontraron registros de <strong>Seguros</strong></h3>
        </div>
    @endif
</div>
@section('scripts')
    ...
    <script>
        ...
        function botonAceptar(id){
            ...
            Swal.fire({
                title: "¿Estas seguro que quieres eliminar este seguro?",
        ...
        function getDelete(id) {
            // Ruta de la peticion
            const url = '{{route("seguros.delete")}}'
            // Peticion
            return $.ajax({
                type: "POST",
                url: url,
