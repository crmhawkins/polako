...
<div>
    ...
    @if ( $ocas )
        ...
        <div class="table-responsive">
             <table class="table table-hover">
                ...
                <tbody>
                    @foreach ( $ocas as $oca )
                    <tr class="clickable-row" data-href="{{route('ocas.edit', $oca->id)}}">
                            <td>{{$oca->alta ? \Carbon\Carbon::parse($oca->alta)->format('d/m/Y') : 'N/A'}}</td>
                            <td>{{$oca->caducidad ? \Carbon\Carbon::parse($oca->caducidad)->format('d/m/Y') : 'N/A'}}</td>
                            <td>{{$oca->descripcion}}</td>
                            <td>{{optional($oca->adminUser)->name}}</td>
                            <td>{{optional($oca->salon)->nombre}}</td>
                            <td class="flex flex-row justify-evenly align-middle" style="min-width: 120px">
                                <a class="" href="{{route('ocas.edit', $oca->id)}}"><img src="{{asset('assets/icons/edit.svg')}}" alt="Editar oca"></a>
                                <a class="delete" data-id="{{$oca->id}}" href=""><img src="{{asset('assets/icons/trash.svg')}}" alt="Eliminar oca"></a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
    ...
    @else
        <div class="text-center py-4">
            <h3 class="text-center fs-3">No se encontraron registros de <strong>Oca</strong></h3>
        </div>
    @endif
</div>
@section('scripts')
